...
Considerando los problemas citados anteriormente y las restricciones impuestas por el proyecto \ac{RASimAs} (ver apartado \ref{posing:discusion}), esta tesis pretende comprobar si es posible diseñar un método geométrico que permita \new{reposicionar pacientes virtuales que se utilizarán para entrenar en un entorno médico de forma efectiva}. A continuación, se muestran las diferencias entre ambas aproximaciones:

\begin{itemize}
\item Los métodos basados en modelos físicos se centran en conseguir deformaciones plausibles, frente a los geométricos que proporcionan soluciones \new{creibles}\del{plausibles}. Es decir, deformaciones que el usuario pueda interpretar como reales. 
...
\item Por otro lado, los métodos basados física requieren caracterizar mecánicamente los \new{tejidos} que se van a simular. Está información no siempre puede obtenerse de forma precisa a partir de las imágenes médicas.
...
%\todo{Su flexibilidad y su rapidez le hacen posible incorporarlo en cualquier simulador o herramienta de \ac{RV}. REDACTA BIEN}
    \item \new{En el contexto del entrenamiento médico, poder incorporarlo en cualquier simulador o herramienta de \ac{RV} gracias a su flexibilidad y sus tasas de refresco interactivas.} 
\end{itemize}
