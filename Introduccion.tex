...
\begin{itemize}
    ...
    \item \textbf{Dificultad de obtener modelos anatómicos adecuados:} En la actualidad, ninguna técnica de imagen médica (\ac{US}, \ac{TC}, \ac{IRM}...) es capaz de obtener descripciones completas de un paciente \cite{ferrante2017slice}\todo{buscar cita OK, NO PONEMOS CITA, esta la uso en el estado del arte, pero es de registro de imágenes, en donde explica que algunas técnicas usan más de una imagen}. Por otro lado, las imágenes obtenidas deben adaptarse a representaciones que puedan utilizarse durante la simulación.
\end{itemize}
...
Así mismo, el simulador se compone de varios módulos software que se encargan de la simulación física del sistema\todo{Tal y como está escrita esta frase parece que todos los módulos SW se encargan de la simulación.}. 
...
\todo{Esto puede ir al capitulo de RASimAs} El usuario puede elegir entre modos de entrenamiento, donde la  aplicación registrará métricas de rendimiento con el objetivo de proporcionar retroalimentación sumativa y formativa. 
%
...
Con el objetivo de enfrentar a los médicos a distintos escenarios que representen una variabilidad anatómica significativa, en el proyecto \ac{RASimAs} se propuso desarrollar un conjunto de herramientas con las que generar una base de datos de pacientes virtuales. \del{Este paquete de aplicaciones \del{desarrolladas con distintos objetivos y capaces de cooperar entre sí} permite la creación de pacientes virtuales medios que se utilizarán en \ac{RASim} y \ac{RAAs}}\todo{No aporta nada y no se utilzan en RAAs}. 
%
